...
export const updateUserData = async (userId, updatedData) => {
  try {
    ...
    // Store only essential fields for audit trail (avoid storing dataHistory recursively)
    const dataHistory = currentData.dataHistory || [];
    // Limit history to last 10 entries to prevent bloat
    if (dataHistory.length >= 10) {
      dataHistory.shift(); // Remove oldest entry
    }
    dataHistory.push({
